...
function PrecisionParity() {
  ...
  const activeMarket = scan.best?.market || "R_100";
  const activeTicks = scan.getTicks(activeMarket);
  const diagnostic =
    activeTicks.length >= 10 ? buildPrecisionParitySignal(activeTicks, activeMarket) : null;
  const activeSignal = diagnostic?.signal ?? scan.best?.signal;
    ...
    <div className="min-h-screen grid-bg text-foreground">
      ...
      <main className="max-w-[1800px] mx-auto px-5 py-5 space-y-5">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-5">
          ...
          {/* TAB 1: OPPORTUNITY (Clean Sentinel View: High-Conviction Signal + Entry Card + Collapsible Accordions) */}
          <TabsContent value="opportunity" className="space-y-5 focus-visible:outline-none">
            {/* Primary Signal Display */}
            {activeSignal && <InstitutionalSignalCard signal={activeSignal} />}

            {/* Actionable Entry Execution Card (Step-by-Step + Variable-Order Markov + 2-Min Lock) */}
            {scan.best ? (
          ...
          <TabsContent value="dbot" className="space-y-5 focus-visible:outline-none">
            ...
            {scan.best && <FinalParityOutput report={scan.best} />}
          </TabsContent>
