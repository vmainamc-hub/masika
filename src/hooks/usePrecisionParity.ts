...
export function usePrecisionParity(
  ...
  const [state, setState] = useState<Omit<ParityState, "scanNow" | "releaseHold" | "getTicks">>({
    markets: [],
    best: null,
    held: null,
    history: [],
  ...
  const runScan = useCallback(() => {
    ...
    const readyMarkets = markets.filter(
      (m) => m.verdict.state === "READY" && m.verdict.recommendation !== "NO_TRADE",
    );
  ...
  return { ...state, scanNow: runScan, getTicks };
}
